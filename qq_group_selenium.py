...
def login_1(username, password):
    ...
    for x in range(int(all_group)):
        ...
        group_1(driver,group_list[x])
    driver.quit()


# 切换群
def group_1(driver,group):
    ...
    with open(filename, 'r+', encoding='utf-8') as f:
        ...
        for x in qq_1:
            if x != '\n':
                ...
                pool.apply_async(login_1,args=(qq_2.group(1),qq_2.group(2))) 
                #login_1(username=qq_2.group(1), password=qq_2.group(2))
        print ('等待所有子进程执行……')
        pool.close()
        pool.join()
        print("子进程执行完毕")
